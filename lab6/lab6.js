...
function testMail(telephone,mail) {
    var istelephone = "wrong",ismail = "wrong";
    ...
    console.log("The telephone is " + istelephone + " and the mail is " + ismail);
}
/*
3.
要求：
...
function lengthOfLongestSubstring(str) {
    ...
    for (i = 0, j = 0; i < str.length; i++) {
        if (list.has(str[i])) {
            j = j < list.get(str[i]) ? list.get(str[i]) : j;
        }
        list.set(str[i], i);
        max_length = max_length < list.size ? (i - j + 1) : max_length;
    }
